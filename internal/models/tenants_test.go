...
func testTenantsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(tenantPrimaryKeyColumns, tenantColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testTenantToManyDeadLetterMessages(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, deadLetterMessageDBTypes, false, deadLetterMessageColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, deadLetterMessageDBTypes, false, deadLetterMessageColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.DeadLetterMessages().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := TenantSlice{&a}
	if err = a.L.LoadDeadLetterMessages(ctx, tx, false, (*[]*Tenant)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DeadLetterMessages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.DeadLetterMessages = nil
	if err = a.L.LoadDeadLetterMessages(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DeadLetterMessages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTenantToManyMessageProcessingLogs(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, messageProcessingLogDBTypes, false, messageProcessingLogColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, messageProcessingLogDBTypes, false, messageProcessingLogColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.MessageProcessingLogs().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := TenantSlice{&a}
	if err = a.L.LoadMessageProcessingLogs(ctx, tx, false, (*[]*Tenant)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.MessageProcessingLogs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.MessageProcessingLogs = nil
	if err = a.L.LoadMessageProcessingLogs(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.MessageProcessingLogs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTenantToManyTenantConfigs(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, tenantConfigDBTypes, false, tenantConfigColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, tenantConfigDBTypes, false, tenantConfigColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.TenantConfigs().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := TenantSlice{&a}
	if err = a.L.LoadTenantConfigs(ctx, tx, false, (*[]*Tenant)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TenantConfigs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TenantConfigs = nil
	if err = a.L.LoadTenantConfigs(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TenantConfigs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTenantToManyAddOpDeadLetterMessages(t *testing.T) {
	var err error
	...
	foreigners := []*DeadLetterMessage{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, deadLetterMessageDBTypes, false, strmangle.SetComplement(deadLetterMessagePrimaryKeyColumns, deadLetterMessageColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*DeadLetterMessage{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddDeadLetterMessages(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.DeadLetterMessages[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.DeadLetterMessages[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.DeadLetterMessages().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testTenantToManyAddOpMessageProcessingLogs(t *testing.T) {
	var err error
	...
	var a Tenant
	var b, c, d, e MessageProcessingLog

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, tenantDBTypes, false, strmangle.SetComplement(tenantPrimaryKeyColumns, tenantColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*MessageProcessingLog{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, messageProcessingLogDBTypes, false, strmangle.SetComplement(messageProcessingLogPrimaryKeyColumns, messageProcessingLogColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*MessageProcessingLog{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddMessageProcessingLogs(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.MessageProcessingLogs[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.MessageProcessingLogs[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.MessageProcessingLogs().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testTenantToManyAddOpTenantConfigs(t *testing.T) {
	var err error
	...
	var a Tenant
	var b, c, d, e TenantConfig

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, tenantDBTypes, false, strmangle.SetComplement(tenantPrimaryKeyColumns, tenantColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*TenantConfig{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, tenantConfigDBTypes, false, strmangle.SetComplement(tenantConfigPrimaryKeyColumns, tenantConfigColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*TenantConfig{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddTenantConfigs(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.TenantConfigs[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TenantConfigs[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TenantConfigs().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	tenantDBTypes = map[string]string{`ID`: `uuid`, `Name`: `character varying`, `Status`: `character varying`, `MaxWorkers`: `integer`, `CurrentWorkers`: `integer`, `QueueName`: `character varying`, `ConsumerTag`: `character varying`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_             = bytes.MinRead
)

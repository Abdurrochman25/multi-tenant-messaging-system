...
var MessageTableColumns = struct {
	...
	RetryCount:  "messages.retry_count",
	MaxRetries:  "messages.max_retries",
	ScheduledAt: "messages.scheduled_at",
	ProcessedAt: "messages.processed_at",
...
var MessageWhere = struct {
	...
	TenantID:    whereHelperstring{field: "\"messages\".\"tenant_id\""},
	Payload:     whereHelpertypes_JSON{field: "\"messages\".\"payload\""},
	Status:      whereHelpernull_String{field: "\"messages\".\"status\""},
	RetryCount:  whereHelpernull_Int{field: "\"messages\".\"retry_count\""},
	MaxRetries:  whereHelpernull_Int{field: "\"messages\".\"max_retries\""},
	ScheduledAt: whereHelpernull_Time{field: "\"messages\".\"scheduled_at\""},
	ProcessedAt: whereHelpernull_Time{field: "\"messages\".\"processed_at\""},
...
var (
	messageAllColumns            = []string{"id", "tenant_id", "payload", "status", "retry_count", "max_retries", "scheduled_at", "processed_at", "created_at"}
	messageColumnsWithoutDefault = []string{"tenant_id", "payload"}
	messageColumnsWithDefault    = []string{"id", "status", "retry_count", "max_retries", "scheduled_at", "processed_at", "created_at"}
	messagePrimaryKeyColumns     = []string{"id", "tenant_id"}
...
func FindMessage(ctx context.Context, exec boil.ContextExecutor, iD string, tenantID string, selectCols ...string) (*Message, error) {
	messageObj := &Message{}
	...
	query := fmt.Sprintf(
		"select %s from \"messages\" where \"id\"=$1 AND \"tenant_id\"=$2", sel,
	)

	q := queries.Raw(query, iD, tenantID)
...
func (o *Message) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"messages\" WHERE \"id\"=$1 AND \"tenant_id\"=$2"
...
func (o *Message) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindMessage(ctx, exec, o.ID, o.TenantID)
	if err != nil {
		return err
	}
...
func MessageExists(ctx context.Context, exec boil.ContextExecutor, iD string, tenantID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"messages\" where \"id\"=$1 AND \"tenant_id\"=$2 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, iD, tenantID)
	}
	row := exec.QueryRowContext(ctx, sql, iD, tenantID)

...
func testMessagesExists(t *testing.T) {
	...
	e, err := MessageExists(ctx, tx, o.ID, o.TenantID)
	if err != nil {
...
func testMessagesFind(t *testing.T) {
	...
	messageFound, err := FindMessage(ctx, tx, o.ID, o.TenantID)
	if err != nil {
		t.Error(err)
	}
...
var (
	messageDBTypes = map[string]string{`ID`: `uuid`, `TenantID`: `uuid`, `Payload`: `jsonb`, `Status`: `character varying`, `RetryCount`: `integer`, `MaxRetries`: `integer`, `ScheduledAt`: `timestamp with time zone`, `ProcessedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`}
	_              = bytes.MinRead
)
